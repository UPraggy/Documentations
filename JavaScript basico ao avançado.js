...
/*                                      ########################################################### 
    ...
    /*o var pode ser chamada fora do escopo de outras funções, 
        desde que ela seja usada depois da sua declaração
    EXEMPLO:*/
    ...
    /*o let pode ser chamada apenas dentro do escopo da função
        É a mais recomendada para usar
    EXEMPLO:*/
...
/* Mesmo que na declaração de uma variavel, ela não tenha um tipo especifico, é de 
grande importancia aprender sobre os diferentes tipos de dados para algumas operações */
...
/*Os tipo de dados são os seguintes*/
    ...
    variavel_booleana = false
                                    ...
                                    /*-------------------Null------------------*/
                     /* Apesar de Null ser igual a Undefined, eles não são identicos
                     Null corresponde a um valor vazio
                     Enquanto Undefined transmite que a variavel não existe*/
...
/* Em projetos maiores, normalmente não é usada a impressão, porém em menores ou até na criação dos maiores
a impressão de textos, variaveis e saídas tem uma grande importância */
...
/*                                      ########################################################### 
                                        ...
                                        ###########################################################
                                        ###########################################################*/
/* Um array serve para armazenar mais de uma variavel ou grupo de de dados, um array é uma matriz
onde ela pode mudar de dimensão além disso o JavaScript permite armazenar qualquer tipo de dados dentro dela,
desde tipos basicos, até funções e objetos.
*/
...
/* Um objeto serve para armazenar variaveis em conjuntos de chave/valor além disso o 
JavaScript permite armazenar qualquer tipo de dados dentro dela,
desde tipos basicos, até funções e objetos.
*/
...
/* Funções é um bloco de codigo pronto e são executadas sempre que solicitadas. 
...
/* Classes é nada mais doque uma difinição de objeto, com seus atributos e metodos,
podendo ser 'instanciado'(instanciar é como criar um novo, com base em um modelo) quantas vezes for preciso */
...
/* No ambiente de desenvolvimento, é comum aparecerem alguns erros e exceções por erro ao codificar, 
porém algumas vezes o erro pode vir do usuario ou até mesmo do próprio programador, e alguns tipo de erros
são ncessarios serem tratados, no JavaScript são usadas as palavras-chave 'try, catch e throw' 
...
/* Ótimo para usar ao iterar sobre uma estrutura de dados, mais simples que o for 
covencional */
...
/* Funções pode ter mais de um parametro declarado, e há uma forma de pegar todos os parametros 
...
/* A palavra reservada 'this', serve para referenciar o contexto onde está a função ou parametro,
porém ele pode variar e para impedir essa variação a função 'bind' serve para isso 
Abaixo um exemplo com o 'this' com erro e ele sendo corrigido pelo 'bind':*/
...
/* Mas ao 'instanciar' a função do objeto em outra variavel, ele muda, isso se deve ao
'this' apontar para a nova variavel */
...
/*Obs.: o arrow é tão forte que caso use o bind para referenciar outro objeto, o this ainda vai referenciar
no caso a função Gato, o arrow sempre aponta para o contexto em que foi definido*/
...
/* Funções também podem ser instanciadas como classes, elas em geral são iguais a uma classe, 
porém tem uma forma diferente de serem escritas */
...
/*                                      ########################################################### 
                                        ...
                                        ###########################################################
                                        ###########################################################*/
/* JSON é uma forma de armazenar objetos, através de uma string, amplamente usado 
para comunicação entre linguagens, só aceita valores (numero, string, bool,
array, objetos dentro de objetos), funções não*/
...
/*A função Object.assign permite 'concatenar' ,ou seja, unir todos os atributos de diferentes objetos,
em um
vale ressaltar que atributos com mesmas chaves, serão sobreescritos pelo objeto mais recente*/
...
/* Em js para herança é usado a palavra chave '__proto__', para estabelecer um prototipo,
ou seja, o '__proto__' é como uma seta que aponta para o pai de determinado objeto,
com esse apontamento, caso o filho não tenha algum atributo e o pai tiver, é possivel
acessar através do filho o atributo (valor ou função) do pai, abaixo um exemplo de utilização:
*/
...
/*Seguindo essa linha mesmo assim como o objeto tem o 'this' para referencia-lo,
há também o 'super' usado para referenciar o objeto pai
no exemplo abaixo, a função é chamada, e dentro dela é chamada como complemento a função pai 
utilizando o 'super' verfique os metods mais acima para melhor entendimento
*/
cachorro.status();
...
/*
A função reduce, reduz um array em um valor final, basicamente ele pega o primeiro valor
e a partir dele faz uma operação, onde ele pega o resultado da operação, e realiza ela com o proximo elemento
do array,
por exemplo uma soma:
dado um array -> [1,2,4,2,5,1]
eu faço o reduce somando os elementos
resultado+elemento
ai ele pega o primeiro indice realiza a operação 
0+1 e chama a callback de novo
agora pega o segundo e soma com o resultado anterior
1+2 -> 3
e vai até o ultimo
3+4
7+2
9+5
14+1
por fim retorna o resultado 15
*/
